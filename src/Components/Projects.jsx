...
const Projects = () => 
{
  ...
  return (
    <section id="projects" className="relative min-h-screen py-10 px-6 bg-[#0d2137]">
              ...
              <motion.div key={project.title} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: false }} className="group flex flex-col bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:border-cyan-500/50 hover:bg-white/10 transition-all duration-300">
                ...
                <div className="p-5 flex-1">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-2.5 py-1 text-xs bg-blue-500/20 text-cyan-400 rounded-full border border-blue-500/30">{tag}</span>
                    ))}
                  </div>
                </div>
                ...
                <div className="px-5 pb-5 shrink-0">
                  <div className="flex gap-3">
                    <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 bg-linear-to-r from-blue-500 to-cyan-500 text-white text-sm font-medium rounded-lg hover:shadow-lg hover:shadow-cyan-500/30 transition-all duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                      </svg>
                      Live Demo
                    </a>
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 py-2.5 px-4 border border-white/20 bg-white/5 text-white text-sm font-medium rounded-lg hover:bg-white/10 hover:border-white/40 transition-all duration-300">
            ...
            <div className="flex justify-center items-center gap-2 mt-12">
...

              {
                [...Array(totalPages)].map((_, index) => 
                {
                  ...
                  return (
                    <button key={pageNumber} onClick={() => paginate(pageNumber)} className={`w-10 h-10 rounded-lg font-medium transition-all duration-300 ${currentPage === pageNumber ? "bg-linear-to-r from-blue-500 to-cyan-500 text-white" : "bg-white/5 border border-white/10 text-white hover:bg-white/10 hover:border-cyan-500/50"}`}>
                      {pageNumber}
                    </button>
                  )
                })
